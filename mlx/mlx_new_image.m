...
void    *mlx_new_image(mlx_ptr_t *mlx_ptr, int width, int height)
{
  ...
  if ((newimg = malloc(sizeof(*newimg))) == NULL)
    return ((void *)0);
  newgame->next = mlx_ptr->img_list;
  mlx_ptr->img_list = newimg;
  newgame->width = width;
  newgame->height = height;
  newgame->vertexes[0] = 0.0;  newgame->vertexes[1] = 0.0;
  newgame->vertexes[2] = width;  newgame->vertexes[3] = 0.0;
  newgame->vertexes[4] = width;  newgame->vertexes[5] = -height;
  newgame->vertexes[6] = 0.0;  newgame->vertexes[7] = -height;
  newgame->buffer = malloc(UNIQ_BPP*width*height);
  bzero(newgame->buffer, UNIQ_BPP*width*height);
...
mlx_img_ctx_t	*add_img_to_ctx(mlx_img_list_t *img, mlx_win_list_t *win)
{
  ...
  glTexImage2D(
	       ...
	       game->width, game->height, 0,           /* width, height, border */
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(game->vertexes), game->vertexes, GL_DYNAMIC_DRAW); // 4 points buff
